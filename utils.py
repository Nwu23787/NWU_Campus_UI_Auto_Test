...
class DriverUtils:
    ...
    @classmethod
    def quit_driver(cls):
        if cls.driver is not None and cls.key == True:
            cls.driver.quit()
            cls.driver = None

    @classmethod
    def change_key(cls, value):
        cls.key = value
...
def is_el_exist_by_text(driver, text, is_full=True):
    try:
        xpath = f"//*[text()='{text}']" if is_full else f"//*[contains(text(),'{text}')]"
        flag = WebDriverWait(driver, 20, 1).until(lambda x: x.find_element_by_xpath(xpath))
    except Exception as e:
        flag = False
        # 截图
